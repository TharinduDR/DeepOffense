...
from examples.hindi.global_args import global_args

model = ClassificationModel("bert", "bert-base-multilingual-cased",  use_cuda=torch.cuda.is_available(),
                           args=global_args)

data = pd.read_csv('examples/hindi/data/hindi_dataset.tsv', sep="\t",  quoting=csv.QUOTE_NONE)
data = data.rename(columns={'task_1': 'label'}).dropna()
